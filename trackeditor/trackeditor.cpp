	...
	: QDialog(parent)
{
	...
	for (int row = 0; row < tracks->count(); ++row)
	{
		...
		item0 = new QTableWidgetItem(track.title);
		tableWidget->setItem(row, 0, item0);

		item1 = new QTableWidgetItem(QString::number(track.duration));
	...
	addTrackButton = buttonBox->addButton(tr("&Add Track"), QDialogButtonBox::ActionRole);
